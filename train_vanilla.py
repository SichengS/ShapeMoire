...
def validate_epoch(args, ValidateImgLoader, model, validate_model_fn, compute_metrics):
    tbar = tqdm(ValidateImgLoader)
    total_psnr = 0
    
    for batch_idx, data in enumerate(tbar):
        model.eval()
        cur_psnr = validate_model_fn(args, data, model, compute_metrics)
        number = data['number']
...
def load_checkpoint(model, optimizer, load_epoch):
    load_dir = '/home/sshen/UHDM/out_dir/uhdm/ShapeMoire/ShapeConv/net_checkpoints/checkpoint_000070.tar'
...
def main():
    ...
    model_fn, validate_model_fn = model_fn_decorator(loss_fn=loss_fn, device=device)
    # create function for calculating psnr
    ...
    args.BATCH_SIZE = 1
    ValidateImgLoader = create_dataset(args, data_path=Validate_path, mode='test')
    ...
    for epoch in range(args.LOAD_EPOCH + 1, args.EPOCHS + 1):
        ...
        # validate
        cur_psnr = validate_epoch(args, ValidateImgLoader, model, validate_model_fn, compute_metrics)
